...
pub async fn get_courses_for_teacher_db(
    pool: &PgPool,
    teacher_id: i32,
) -> Result<Vec<Course>, WebServiceError> {
...
pub async fn get_course_details_db(
    ...
    if let Ok(course) = row {
        ...
        Err(WebServiceError::NotFound("Course didn't founded".into()))
    }
}
...
pub async fn update_course_details_db(
    ...
    .fetch_one(pool)
    .await
    .map_err(|_err| WebServiceError::NotFound("Course Id not found".into()))?;
    ...
    if let Ok(course) = course_row {
        ...
        Err(WebServiceError::NotFound("Course id not found".into()))
    }
}
